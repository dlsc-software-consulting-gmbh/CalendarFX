...
class RRuleSchema extends IcalSchema {
    ...
    static {
        Map<String, ParamRule> paramRules = new HashMap<>();
        Map<String, ContentRule> contentRules = new HashMap<>();
        Map<String, ObjectRule> objectRules = new HashMap<>();
        Map<String, XformRule> xformRules = new HashMap<>();

        // rrule      = "RRULE" rrulparam ":" recur CRLF
        // exrule     = "EXRULE" exrparam ":" recur CRLF
        objectRules.put("RRULE", (schema, params, content, target) -> {
            schema.applyParamsSchema("rrulparam", params, target);
            schema.applyContentSchema("recur", content, target);
        });
        objectRules.put("EXRULE", (schema, params, content, target) -> {
        ...
        // rrulparam  = *(";" xparam)
        // exrparam   = *(";" xparam)
        ParamRule xparamsOnly = (schema, name, value, out) -> schema.badParam(name, value);
         ...
         *            )
         */
        contentRules.put("recur", (schema, content, target) -> {
        ...
        objectRules.put("EXDATE", (schema, params, content, target) -> {
        ...
        // "FREQ"=freq *(
        contentRules.put("FREQ", (schema, value, target) -> ((RRule) target).setFreq(
                (Frequency) schema.applyXformSchema("freq", value)));

        //  ( ";" "UNTIL" "=" enddate ) /
        contentRules.put("UNTIL", (schema, value, target) -> ((RRule) target).setUntil(
                (DateValue) schema.applyXformSchema("enddate", value)));

        //  ( ";" "COUNT" "=" 1*DIGIT ) /
        contentRules.put("COUNT", (schema, value, target) -> ((RRule) target).setCount(Integer.parseInt(value)));

        //  ( ";" "INTERVAL" "=" 1*DIGIT )          /
        contentRules.put("INTERVAL", (schema, value, target) -> ((RRule) target).setInterval(Integer.parseInt(value)));

        //  ( ";" "BYSECOND" "=" byseclist )        /
        contentRules.put("BYSECOND", (schema, value, target) -> ((RRule) target).setBySecond(
                (int[]) schema.applyXformSchema("byseclist", value)));

        //  ( ";" "BYMINUTE" "=" byminlist )        /
        contentRules.put("BYMINUTE", (schema, value, target) -> ((RRule) target).setByMinute(
                (int[]) schema.applyXformSchema("byminlist", value)));

        //  ( ";" "BYHOUR" "=" byhrlist )           /
        contentRules.put("BYHOUR", (schema, value, target) -> ((RRule) target).setByHour(
                (int[]) schema.applyXformSchema("byhrlist", value)));

        //  ( ";" "BYDAY" "=" bywdaylist )          /
        contentRules.put("BYDAY", (schema, value, target) -> ((RRule) target).setByDay(
                (List<WeekdayNum>) schema.applyXformSchema("bywdaylist", value)));

        //  ( ";" "BYMONTHDAY" "=" bymodaylist )    /
        contentRules.put("BYMONTHDAY", (schema, value, target) -> ((RRule) target).setByMonthDay(
                (int[]) schema.applyXformSchema("bymodaylist", value)));

        //  ( ";" "BYYEARDAY" "=" byyrdaylist )     /
        contentRules.put("BYYEARDAY", (schema, value, target) -> ((RRule) target).setByYearDay(
                (int[]) schema.applyXformSchema("byyrdaylist", value)));

        //  ( ";" "BYWEEKNO" "=" bywknolist )       /
        contentRules.put("BYWEEKNO", (schema, value, target) -> ((RRule) target).setByWeekNo(
                (int[]) schema.applyXformSchema("bywknolist", value)));

        //  ( ";" "BYMONTH" "=" bymolist )          /
        contentRules.put("BYMONTH", (schema, value, target) -> ((RRule) target).setByMonth(
                (int[]) schema.applyXformSchema("bymolist", value)));

        //  ( ";" "BYSETPOS" "=" bysplist )         /
        contentRules.put("BYSETPOS", (schema, value, target) -> ((RRule) target).setBySetPos(
                (int[]) schema.applyXformSchema("bysplist", value)));

        //  ( ";" "WKST" "=" weekday )              /
        contentRules.put("WKST", (schema, value, target) -> ((RRule) target).setWkSt(
                (Weekday) schema.applyXformSchema("weekday", value)));

        // freq       = "SECONDLY" / "MINUTELY" / "HOURLY" / "DAILY"
        //            / "WEEKLY" / "MONTHLY" / "YEARLY"
        xformRules.put("freq", (schema, value) -> Frequency.valueOf(value));

        // enddate    = date
        // enddate    =/ date-time            ;An UTC value
        xformRules.put("enddate", (schema, value) -> IcalParseUtil.parseDateValue(value.toUpperCase()));

        // byseclist  = seconds / ( seconds *("," seconds) )
        // seconds    = 1DIGIT / 2DIGIT       ;0 to 59
        xformRules.put("byseclist", (schema, value) -> parseUnsignedIntList(value, 0, 59, schema));

        // byminlist  = minutes / ( minutes *("," minutes) )
        // minutes    = 1DIGIT / 2DIGIT       ;0 to 59
        xformRules.put("byminlist", (schema, value) -> parseUnsignedIntList(value, 0, 59, schema));

        // byhrlist   = hour / ( hour *("," hour) )
        // hour       = 1DIGIT / 2DIGIT       ;0 to 23
        xformRules.put("byhrlist", (schema, value) -> parseUnsignedIntList(value, 0, 23, schema));
        ...
        xformRules.put("bywdaylist", (schema, value) -> {
            String[] parts = COMMA.split(value);
            List<WeekdayNum> weekdays = new ArrayList<>(parts.length);
            for (String p : parts) {
        ...
        xformRules.put("weekday", (schema, value) -> Weekday.valueOf(value.toUpperCase()));

        // bymodaylist = monthdaynum / ( monthdaynum *("," monthdaynum) )
        // monthdaynum = ([plus] ordmoday) / (minus ordmoday)
        // ordmoday   = 1DIGIT / 2DIGIT       ;1 to 31
        xformRules.put("bymodaylist", (schema, value) -> parseIntList(value, 1, 31, schema));

        // byyrdaylist = yeardaynum / ( yeardaynum *("," yeardaynum) )
        // yeardaynum = ([plus] ordyrday) / (minus ordyrday)
        // ordyrday   = 1DIGIT / 2DIGIT / 3DIGIT      ;1 to 366
        xformRules.put("byyrdaylist", (schema, value) -> parseIntList(value, 1, 366, schema));

        // bywknolist = weeknum / ( weeknum *("," weeknum) )
        // weeknum    = ([plus] ordwk) / (minus ordwk)
        xformRules.put("bywknolist", (schema, value) -> parseIntList(value, 1, 53, schema));

        // bymolist   = monthnum / ( monthnum *("," monthnum) )
        // monthnum   = 1DIGIT / 2DIGIT       ;1 to 12
        xformRules.put("bymolist", (schema, value) -> parseIntList(value, 1, 12, schema));

        // bysplist   = setposday / ( setposday *("," setposday) )
        // setposday  = yeardaynum
        xformRules.put("bysplist", (schema, value) -> parseIntList(value, 1, 366, schema));


        // rdate      = "RDATE" rdtparam ":" rdtval *("," rdtval) CRLF
        objectRules.put("RDATE", (schema, params, content, target) -> {
        ...
        contentRules.put("rdtval", (schema, content, target) -> {
